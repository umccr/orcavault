...
with legacy_source as (
    ...
    select distinct project_name as project_id, 'data_portal_labmetadata'  as record_source from {{ source('data_portal', 'legacy_data_portal_labmetadata') }}
    union all
    select distinct project_name as project_id, 'data_portal_limsrow'      as record_source from {{ source('data_portal', 'legacy_data_portal_limsrow') }}
    union all
    select distinct project_name as project_id, 'spreadsheet__google_lims' as record_source from {{ ref('spreadsheet__google_lims') }}
...
{# Partitions on project_id with full priority ordering.        #}
...
merged as (
    ...
    {% if var('load_legacy', false) %}
    select project_id, record_source
    from (
        select
            project_id,
            record_source,
        ...
        from (
            select project_id, record_source from legacy_cleaned
            union all
            select project_id, record_source from active_cleaned
        ) combined
    ) t
    where rn = 1
    {% else %}
