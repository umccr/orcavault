{{
    config(
        ...
        dist='external_sample_hk',
        sort=['external_sample_hk', 'load_datetime']
    )
}}
...
with legacy_source as (
    ...
    select distinct external_sample_id, 'data_portal_labmetadata'  as record_source from {{ source('data_portal', 'legacy_data_portal_labmetadata') }}
    union all
    select distinct external_sample_id, 'data_portal_limsrow'      as record_source from {{ source('data_portal', 'legacy_data_portal_limsrow') }}
    union all
    select distinct external_sample_id, 'spreadsheet__google_lims' as record_source from {{ ref('spreadsheet__google_lims') }}
...
{# Partitions on external_sample_id with full priority order.   #}
...
merged as (
    ...
    {% if var('load_legacy', false) %}
    select external_sample_id, record_source
    from (
        select
            external_sample_id,
            record_source,
            row_number() over (
                partition by external_sample_id
        ...
        from (
            select external_sample_id, record_source from legacy_cleaned
            union all
            select external_sample_id, record_source from active_cleaned
        ) combined
    ) t
    where rn = 1
    {% else %}
