...
with legacy_source as (
    ...
    select distinct subject_id    as internal_subject_id, 'data_portal_labmetadata'  as record_source from {{ source('data_portal', 'legacy_data_portal_labmetadata') }}
    union all
    select distinct subject_id    as internal_subject_id, 'data_portal_limsrow'      as record_source from {{ source('data_portal', 'legacy_data_portal_limsrow') }}
    union all
    select distinct subject_id    as internal_subject_id, 'spreadsheet__google_lims' as record_source from {{ ref('spreadsheet__google_lims') }}
...
{# Partitions on internal_subject_id with full priority order.  #}
...
merged as (
    ...
    {% if var('load_legacy', false) %}
    select internal_subject_id, record_source
    from (
        select
            internal_subject_id,
            record_source,
            row_number() over (
                partition by internal_subject_id
        ...
        from (
            select internal_subject_id, record_source from legacy_cleaned
            union all
            select internal_subject_id, record_source from active_cleaned
        ) combined
    ) t
    where rn = 1
    {% else %}
