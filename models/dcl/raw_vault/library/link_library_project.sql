...
with legacy_source as (
    ...
    select distinct library_id, project_name as project_id, 'legacy_data_portal_labmetadata'  as record_source from {{ source('data_portal', 'legacy_data_portal_labmetadata') }}
    union all
    select distinct library_id, project_name as project_id, 'legacy_data_portal_limsrow'      as record_source from {{ source('data_portal', 'legacy_data_portal_limsrow') }}
    union all
    select distinct library_id, project_name as project_id, 'spreadsheet__google_lims' as record_source from {{ ref('spreadsheet__google_lims') }}

),

legacy_cleaned as (

    select distinct library_id, project_id, record_source
    from legacy_source
...
{# CDC incremental watermark on                                 #}
{# app_libraryprojectlink._dms_cdc_timestamp.                   #}
{# ============================================================ #}
...
active_cleaned as (
    ...
    select distinct library_id, project_id, record_source
    from (
        select library_id, project_id, record_source from spreadsheet_source
        union all
        select library_id, project_id, record_source from cdc_source
    ) combined
    where library_id is not null
      and library_id <> ''
      and project_id is not null
...
merged as (
    ...
    {% if var('load_legacy', false) %}
    select library_id, project_id, record_source
    from (
        select
            library_id,
            project_id,
            record_source,
            row_number() over (
                partition by library_id, project_id
        ...
        from (
            select library_id, project_id, record_source from legacy_cleaned
            union all
            select library_id, project_id, record_source from active_cleaned
        ) combined
    ) t
    where rn = 1
    {% else %}
...
final as (
    ...
    where not exists (
        ...
        where t.project_hk = transformed.project_hk
