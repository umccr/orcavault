...
with legacy_source as (
    ...
    select distinct library_id, project_owner as owner_id, 'legacy_data_portal_labmetadata'  as record_source from {{ source('data_portal', 'legacy_data_portal_labmetadata') }}
    union all
    select distinct library_id, project_owner as owner_id, 'legacy_data_portal_limsrow'      as record_source from {{ source('data_portal', 'legacy_data_portal_limsrow') }}
    union all
    select distinct library_id, project_owner as owner_id, 'spreadsheet__google_lims' as record_source from {{ ref('spreadsheet__google_lims') }}

),

legacy_cleaned as (

    select distinct library_id, owner_id, record_source
    from legacy_source
...
{# Spreadsheet PSA layer only — no CDC source for ownership     #}
{# link via library in metadata manager.                        #}
{# ============================================================ #}
...
merged as (
    ...
    {% if var('load_legacy', false) %}
    select library_id, owner_id, record_source
    from (
        select
            library_id,
            owner_id,
            record_source,
            row_number() over (
                partition by library_id, owner_id
        ...
        from (
            select library_id, owner_id, record_source from legacy_cleaned
            union all
            select library_id, owner_id, record_source from active_cleaned
        ) combined
    ) t
    where rn = 1
    {% else %}
