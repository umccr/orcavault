...
with classified as (
    ...
    select
        ...
        filename,
        case
            when ext2 = 'vcf' and ext1 = 'gz' then 'vcf.gz'
            when ext3 = 'vcf' and ext2 = 'gz' and ext1 = 'tbi' then 'vcf.gz.tbi'
        end                                                                 as format
    from
        {{ ref('sat_s3object_by_library') }}

),

vcf_object as (

    select * from classified where format is not null
...
location1 as (
    ...
    from
        ...
        join vcf_object sat on sat.s3object_hk = hub.s3object_hk
